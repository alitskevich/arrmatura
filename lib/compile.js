...
export function placeholder(expr) {
  ...
  return !pipes.length 
    ? c => initGettr(c) 
    : (pipec => c => pipec(c, initGettr(c)))(withPipes(pipes))
}
...
export function compileIf({ tag, attrs, uid, nodes }) {
  ...
  if ((expr.slice(0, 2) === '<-')) {
    ...
    const gttr = parseExpression(expr.includes('{') ? expr : '{' + expr + '}');
    (r.updates || (r.updates = [])).push((c, acc) => { acc['$data'] = neg ? !gttr(c) : gttr(c); });
  }
...
export function compileTag({ attrs, uid, nodes }) {
  ...
  if ((expr.slice(0, 2) === '<-')) {
    ...
    const gttr = parseExpression(expr);
...
export function compile(r) {
  ...
  if (attrs.has('ui:props')) {
    ...
    if ((aProps.slice(0, 2) === '<-')) {
      ...
      const getter = parseExpression(aProps);
