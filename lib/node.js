...
export class Node {
    ...
    get initialState() {
        return this.$initialState || (this.$initialState = {})
    }

    addInitialState(values) {
        Object.entries(values).forEach(([key, val])=> setKeyVal(this.initialState, key, val))
        return this
    }
    ...
    addPropertyConnector(v, getter) {
        ...
        this.initializers.push(c => c.connect(key, rr => (getter(pipec(c.owner, rr)))))
    }
    ...
    addEmitter(v, k) {
        ...
        const pipec = withPipes(pipes)
        this.initializers.push( c => ({payload: { [k]: data => c.emit(expr(c), pipec(c.owner, data))}}))
    } 
    ...
    resolveProps(owner, isInitial) {
        let props = this.$propertyResolvers ? this.$propertyResolvers.reduce((r, fn) => { fn(owner, r); return r }, {}) : {}
