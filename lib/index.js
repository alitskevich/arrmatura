...
function compileAttrs (attrs) {
  ...
  attrs.forEach((v, k) => {
      ...
      return r.push((c, p) => {
        ...
        p['$' + k] = function () {
          ...
          if (url !== old) {
            ...
            setTimeout(() => this.app.subscribe ? this.app.subscribe(url, this, cb) : cb(new Error('No app.subscribe()')), 10)
          }
        }
        return p
      })
    ...
    if (v[0] === '-' && v[1] === '>') {
      ...
      return r.push((c, p) => {
        p[k] = (data) => c.app.dispatch ? c.app.dispatch(fctr(c, {})[k], data) : console.error('No c.app.dispatch()')
        return p
      })
    }
