...
function super_assign (d) {
  this.$ && this.$.__assign__(d)
}

function getAssign (d) {
  return this._bound_assign || (this._bound_assign = (d)=>this.assign(d))
}
...
const register = ctor => {
  ...
  // compile
  const compiled = { factory: () => compile(parseXML(text, name)) }
  function super_render () {
    // lazy template compilation
    const template = compiled.template || (compiled.template = compiled.factory())
    render(this.$, resolve(new Map(), this.$, template), this.$.parentElt)
  }
  ...
  Object.assign(proto, {
    ...
    super_render,
    getAssign,
    assign: proto.assign || super_assign,
    render: proto.render || super_render,
    defer
  })
  // register
