...
export const parseAttrs = (s, r = new Map()) => {
  for (let e = RE_ATTRS.exec(s); e; e = RE_ATTRS.exec(s)) {
    r.set(e[1], !e[2] ? true : Object.parseValue(decodeXmlEntities(skipQoutes(e[2].slice(1)))))
  }
  return r
}
...
export function stringifyNode ({ tag, attrs, nodes }, tab = '') {
  ...
  const ssubs = !nodes ? '' : (nodes.map ? nodes : [...nodes.values()]).map(c => stringifyNode(c, `  ${tab}`)).join('\n')
