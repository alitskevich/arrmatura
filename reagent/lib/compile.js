...
export function placeholder(expr) {
  ...
  const initGettr = (key[0] === ':')
    ? (fn => c => c.res(fn(c)))(stringInterpolation(key.slice(1).trim()))
    : c => c.prop(key)
...
export const createTemplate = (text, name) => text ? compileNode(parseXML(text), name) : []

export const applyTemplate = (ctor, text, name) => {
  ctor.$TEMPLATE = () => {
    try {
      const T = createTemplate(typeof text === 'function' ? text() : text, name);
      ctor.$TEMPLATE = () => T;
      return T;
    } catch (ex) {
